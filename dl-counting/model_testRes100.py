...
# model_path = '11.4-lr-0.005-scaled-batch-100-v15-ResFCN100/'+ 'model.h5'
...
batch = Images
imagelist, densitylist = hf.patch_gen_for_test(batch, 100)
...
for i in range(9):
	image = imagelist[i].reshape(1,100,100,1)
...
density_map = hf.patch_merge_for_display(preds_arr, 256)
...
fig.colorbar(cax)
ax.set_title('Cell image (256x256)')
ax = fig.add_subplot(1,3,2)
density_map = (density_map>0)*density_map
cax=ax.imshow(density_map)
fig.colorbar(cax)
ax.set_title('Estimated density (256x256)')
ax.set_xlabel('Cell count:'+str(pred_count))
ax = fig.add_subplot(1,3,3)
cax=ax.imshow(Images[0,:,:,1])
fig.colorbar(cax)
ax.set_title('Ground truth (256x256)')
...
print([pred_count, real_count])
ax = fig.add_subplot(1,3,1)
cax=ax.imshow(imagelist[0].reshape((100,100)))
fig.colorbar(cax)
...
ax.set_xlabel('Cell count:'+str(pred_count))
ax = fig.add_subplot(1,3,3)
cax=ax.imshow(densitylist[0].reshape((100,100)))
fig.colorbar(cax)
...
for i in range(len(imagelist)):
	image = imagelist[i].reshape(1,100,100,1)
	image = (image - np.mean(image))/np.std(image)
	preds = model.predict(image.reshape(1,100,100,1))
	preds = preds/100
	preds = preds.reshape(100,100)
	preds_list.append(preds)
...
for i in range(len(preds_list)):
	...
	if (i+1)%9 ==0:
		...
		estimate = hf.patch_merge_for_display(preds_arr, 256)
