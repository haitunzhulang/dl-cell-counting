...
# model_path = '10.09-lr-0.005-scaled-batch-100-v11-fcn200/'+ 'model.h5'
# model_path = '10.31-lr-0.005-scaled-batch-80-v11-ResFCN/'+ 'model.h5'
# model_path = '11.1-lr-0.005-scaled-batch-80-v11-WOResFCN/dt-11.3-lr-0.0001-scaled-batch-60-v14-WOResFCN/'+ 'model.h5'
model_path = '10.31-lr-0.005-scaled-batch-80-v11-ResFCN/dt-11.3-lr-1e-05-scaled-batch-60-v14-ResFCN/'+ 'model.h5'
# model_path = '9.26-lr-0.001-scaled-batch-100-v7-fcn-200/'+ 'model.h5'
val_version = 14
...
for i in range(9):
	image = imagelist[i].reshape(1,200,200,1)
...
density_map = hf.patch_merge_for_display(preds_arr, 512)
...
# 	print([pred_count, real_count])
# 	ax = fig.add_subplot(1,3,1)
# 	image = imagelist[i].reshape(200,200)
# 	image = image*(image>15)
# 	cax = ax.imshow(image)
# 	fig.colorbar(cax)
# 	ax.set_title('Cell image(200x200)')
...
for i in range(len(preds_list)):
	temp_preds.append(preds_list[i])
	if (i+1)%9 ==0:
		preds_arr = np.array(temp_preds)
		estimate = hf.patch_merge_for_display(preds_arr, 512)
...
for i in range(len(estimate_list)):
	pd_count_list.append(np.sum(estimate_list[i]))
	rl_count_list.append(np.sum(Y_val[i,:,:,0]))
	

abs_err_list = np.abs((np.array(pd_count_list)-np.array(rl_count_list)))
mean_abs_err = np.mean(abs_err_list)
std_abs_err = np.std(abs_err_list)
print((mean_abs_err, std_abs_err))
hf.plot_cell_counts(model.name, pd_count_list, rl_count_list)
hf.plot_cell_counts_comp(model.name, pd_count_list, pd_count_list, rl_count_list)

## display the prediction for the whole images
fig = plt.figure()
for i in range(len(estimate_list)):
	plt.clf()
	pred_count = pd_count_list[i]
	real_count = rl_count_list[i]
	print([pred_count, real_count])
	estimate = estimate_list[i]
	estimate = (estimate>0)* estimate
	ground_truth = Y_val[i,:,:,0].reshape(512,512)
	estimate = (estimate - np.min(estimate))/(np.max(estimate)-np.min(estimate))*(np.max(ground_truth)-np.min(ground_truth))
	ax = fig.add_subplot(1,3,1)
	image = X_val[i,:,:,0].reshape(512,512)
# 	image = image*(image>15)
	cax = ax.imshow(image)
	fig.colorbar(cax)
	ax.set_title('Cell image(512x512)')
	ax = fig.add_subplot(1,3,2)
	cax = ax.imshow(estimate)
	fig.colorbar(cax)
	ax.set_title('Estimated density(512x512)')
	ax.set_xlabel('Cell count:'+str(pred_count))
	ax = fig.add_subplot(1,3,3)
	cax = ax.imshow(ground_truth)
	fig.colorbar(cax)
	ax.set_title('Ground truth(512x512)')
	ax.set_xlabel('Cell count:'+str(real_count))
	plt.pause(1.0)
