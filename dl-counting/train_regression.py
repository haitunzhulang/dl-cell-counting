...
# Y_train = Y_train * 100
# Y_val = Y_val * 100
nb_pixels = np.sum(X_train.shape[1]*X_train.shape[2])
Y_train = Y_train
Y_val = Y_val

model_name = '11.4-'+'lr-'+str(lr)+ '-scaled'+'-batch-'+str(batch_size)+'-v'+str(val_version)+'-ResLR'
# model_name = '9.30-'+'lr-'+str(lr)+ '-scaled'+'-batch-'+str(batch_size)+'-v'+str(val_version)+'-FCN'
model.name = model_name
hf.train_regression_model(model, X_train,X_val,Y_train,Y_val, nb_epochs=nb_epochs, nb_epoch_per_record=nb_epoch_per_record, input_shape=input_shape[0], batch_size = batch_size)