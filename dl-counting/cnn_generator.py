...
def ResFCN100():
	...
	conv1 = BatchNormalization()(conv1)
	conv1 = Activation('relu')(conv1)
	x1 = Conv2D(nb_filters, kernel_size, padding='same')(conv1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = merge([x1,conv1],mode='sum')
	x2 = Conv2D(nb_filters, kernel_size, padding='same')(x1)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	conv1 = merge([x2,x1],mode='sum')
	pool1 = MaxPooling2D(pool_size=pool_size)(conv1)
...
# 	pool2 = MaxPooling2D(pool_size=pool_size)(conv2)
	...
	conv3 = Conv2D(nb_filters*2, kernel_size, padding='same')(pool1)
	conv3 = BatchNormalization()(conv3)
	conv3 = Activation('relu')(conv3)
	nc = int(nb_filters*2/4)
	ks = (1,1)
	x1 = Conv2D(nc, ks, padding='same')(conv3)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nc, kernel_size, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nb_filters*2, ks, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = merge([x1,conv3],mode='sum')
	x2 = Conv2D(nc, ks, padding='same')(x1)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nc, kernel_size, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nb_filters*2, ks, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	conv3 = merge([x2,x1],mode='sum')
	pool3 = MaxPooling2D(pool_size=pool_size)(conv3)

	conv4 = Conv2D(nb_filters*4, kernel_size, padding='same')(pool3)
	conv4 = BatchNormalization()(conv4)
	conv4 = Activation('relu')(conv4)
	nc = int(nb_filters*4/4)
	ks = (1,1)
	x1 = Conv2D(nc, ks, padding='same')(conv4)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nc, kernel_size, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nb_filters*4, ks, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = merge([x1,conv4],mode='sum')
	x2 = Conv2D(nc, ks, padding='same')(x1)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nc, kernel_size, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nb_filters*4, ks, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	conv4 = merge([x2,x1],mode='sum')
	pool4 = MaxPooling2D(pool_size=pool_size)(conv4)

	conv5 = Conv2D(nb_filters*8, kernel_size, padding='same')(pool4)
	conv5 = BatchNormalization()(conv5)
	conv5 = Activation('relu')(conv5)
	nc = int(nb_filters*8/4)
	ks = (1,1)
	x1 = Conv2D(nc, ks, padding='same')(conv5)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nc, kernel_size, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = Conv2D(nb_filters*8, ks, padding='same')(x1)
	x1 = BatchNormalization()(x1)
	x1 = Activation('relu')(x1)
	x1 = merge([x1,conv5],mode='sum')
	x2 = Conv2D(nc, ks, padding='same')(x1)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nc, kernel_size, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	x2 = Conv2D(nb_filters*8, ks, padding='same')(x2)
	x2 = BatchNormalization()(x2)
	x2 = Activation('relu')(x2)
	conv6 = merge([x2,x1],mode='sum')

	up_conv6 = UpSampling2D(size=pool_size)(conv6)
	ch, cw = get_crop_shape(conv4, up_conv6)
	crop_conv4 = Cropping2D(cropping = (ch,cw))(conv4)
	up7 = concatenate([up_conv6, crop_conv4], axis = concat_axis)
	conv7 = Conv2D(nb_filters*4, kernel_size, padding='same')(up7)
	de_conv7 = Conv2DTranspose(nb_filters*4, kernel_size, strides=(1, 1), padding='same')(conv7)
	de_conv7 = BatchNormalization()(de_conv7)
	de_conv7 = Activation('relu')(de_conv7)
	#     x = ZeroPadding2D(padding=(1, 1))(x)
...
# 	de_conv9 = Activation('relu')(de_conv9)    
	...
	up_conv9 = UpSampling2D(size=pool_size)(de_conv8)
	up10 = concatenate([up_conv9, conv1], axis = concat_axis)
	conv10 = Conv2D(nb_filters*2, kernel_size, padding='same')(up10)
	de_conv10 = Conv2DTranspose(nb_filters, kernel_size, strides=(1, 1), padding='same')(conv10)
	de_conv10 = BatchNormalization()(de_conv10)
	de_conv10 = Activation('relu')(de_conv10)    
...
def WOResFCN():
	## add convolutional layers after concatenation of the features from different layers
	## add residual network
	## remove the residual layers from the ResFCN network for the purpose of comparing 
	## modified at Nov-1-2017
	nb_filters=32
	kernel_size=(3,3)
	pool_size=(2,2)
	nb_dense=512
...
def WOResFCRN():
	...
	## add residual network
	## remove the residual layers from the ResFCN network for the purpose of comparing 
	## remove the skip layers and try to remove the local features
	## FCN network for the purpose of comparing
	## modified at Nov-3-2017
	nb_filters=32
	kernel_size=(3,3)
	pool_size=(2,2)
	nb_dense=512
...
def ResLR():
	...
	## remove the skip layers and try to remove the local features
	## FCN network for the purpose of comparing
	## Linear regression, so remove the decon layers
	## modified at Nov-3-2017
	nb_filters=32
	kernel_size=(3,3)
	pool_size=(2,2)
...
# 	conv6 = x
	...
	gpool = GlobalAveragePooling2D()(conv6)
	
	fc1 = Dense(512, activation = 'relu')(gpool)
	
	fc_output = Dense(1, activation = 'relu')(fc1)
# 	fcn_output = Conv2D(1, kernel_size, padding='same')(de_conv10)

	model = Model(inputs, fc_output)
