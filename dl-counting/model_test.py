...
model_path = '9.17-lr-0.0005-scaled-batch-400v-3fcn-A/'+ 'model.h5'
val_version = 3
...
print([pred_count, real_count])
ax = fig.add_subplot(1,3,1)
ax.imshow(imagelist[0].reshape((100,100)))
...
ax.set_xlabel('Cell count:'+str(pred_count))
ax = fig.add_subplot(1,3,3)
ax.imshow(densitylist[0].reshape((100,100)))
...
file_name = './real.dat'
train_data = np.fromfile(train_path,dtype=np.float,count=250*250,sep='')
train_data = train_data.reshape(250,250)
image = 256*(train_data - np.min(train_data))/(np.max(train_data)-np.min(train_data))
imagelist = []
imagelist.append(image[:100,:100])
imagelist.append(image[:100,100:200])
imagelist.append(image[100:200,:100])
imagelist.append(image[100:200,100:200])
preds_list = []
preds_count_list = []
for i in range(len(imagelist)):
	preds = model.predict(imagelist[i].reshape(1,100,100,1))
